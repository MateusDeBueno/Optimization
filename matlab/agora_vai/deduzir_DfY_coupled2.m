...
% contourLevels = [0 0.9 0.99 1.01 1.02];
...
[cCont, hCont] = contourf(x1*1e3,y1,z1,contourLevels, 'LineStyle', 'none');
...
% colors = f_create_cmap(length(contourLevels), color1, color2);
% [cCont, hCont] = contourf(x2*1e3,y2,z2,contourLevels, 'LineStyle', 'none');
% cmap = interp1(contourLevels, colors, linspace(min(contourLevels), max(contourLevels), 256));
% colormap(cmap);
% leg = contourLegend(hCont);
% leg.FontSize = 14;
...
%%
figure
contourLevels = [-50 -3 0 3 50];
colors = f_create_cmap(length(contourLevels), color1, color2);

hold on
[cCont, hCont] = contourf(Xf1*180/pi, vec_d, Zpp.f1',contourLevels, 'LineStyle', 'none');
[cCont2, hCont2] = contourf(Xf2*180/pi, vec_d, Zpp.f2',contourLevels, 'LineStyle', 'none');
hold off
...
% contourf(Xf1*180/pi, vec_d, Zpp.f1','ShowText', 'on','Color',color1);
% contourf(Xf2*180/pi, vec_d, Zpp.f2','ShowText', 'on','Color',color1);
...
figure
hold on
[C,h] = contourf(X, Y, Zp',[-5 0 5], 'ShowText', 'on','Color',color1);
% h.LineWidth = 1.5;
% clabel(C,h,'FontSize',15,'Color',[0 0 0],'LineWidth',2)
% [C,h] = contour(x2*1e3, y2, z2,[0.94 0.96 0.98 1 1.005 1.01 1.04 1.1], 'ShowText', 'on','Color',color1);
% h.LineWidth = 1.5;

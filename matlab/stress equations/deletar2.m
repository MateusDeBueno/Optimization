...
% intervalo = [0 deg2rad(80)];
intervalo = [deg2rad(10) deg2rad(80)];
    ...
    %malhas
    m_p = Td*u(1:3) == vLdab + vP; %malha primario
    m_s = Td*u(4:6) == vS; %malha secundario
...
%     B = [Bx(1:2,:);Bx(4:5,:)];
    ...
    for ii=1:length(x0s)-1
        ihb(:,ii) = subs(ild(1),x(1:3),x0s(1:3,ii));
        dihb(:,ii) = subs(dild(1),dx(1:3),dx0s(1:3,ii));
    end
    ...
    %corrente no HB
    for ii=1:length(x0s)-1
        HB(:,ii) = subs(iHB(1),x(4:6),x0s(4:6,ii));
        d_HB(:,ii) = subs(diHB(1),dx(4:6),dx0s(4:6,ii));
    end
